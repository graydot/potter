...
class LLMManagerTests: TestBase {
    ...
    func testValidationStates() {
        ...
        XCTAssertFalse(llmManager.validationStates[.anthropic]?.isValid ?? true)
    ...
    func testHasValidProvider() {
        ...
        llmManager.setAPIKey("", for: .anthropic)
    ...
    func testIsProviderConfigured() {
        ...
        XCTAssertFalse(llmManager.isProviderConfigured(.anthropic))

        // Set as valid
        APIKeyService.shared.setValidationStateForTesting(.valid, for: .anthropic)
        XCTAssertTrue(llmManager.isProviderConfigured(.anthropic))
    ...
    func testValidateEmptyAPIKey() async {
        ...
        let state = llmManager.validationStates[.anthropic]
    ...
    func testModelsForCurrentProvider() {
    ...
    func testModelsForProvider() {
        let models = llmManager.modelsForProvider(.anthropic)
        XCTAssertFalse(models.isEmpty)
        XCTAssertTrue(models.allSatisfy { $0.provider == .anthropic })
    }

    func testSelectProviderUsesRegistryModels() {
